...
module.exports = class extends Command {
   ...
   async run ({ channel, guild, mentions, args, author, member }) {
     ...
     if (!mutado) return channel.send(":xSweet: | Por favor diga quem você quer mutar!")
     if (!member.hasPermission("MANAGE_MESSAGES", "ADMINISTRATOR")) return channel.send(":xSweet: | Você não tem permissão para mutar as pessoas!")
     if (mutado.id === author.id) return channel.send(":xSweet: | Você não pode se mutar :v")
     ...
     let time = args[1]
     if (!time) return channel.send(":xSweet: | Diga um tempo por favor")
